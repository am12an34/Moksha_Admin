    ...
    <main class="max-w-6xl mx-auto px-2 sm:px-0 py-6 space-y-6 min-h-[calc(100vh-128px)]">
      <form class="px-1 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4" @submit="search">
        <div class="flex flex-col sm:flex-row gap-2 sm:gap-6">
          <SelectMenu class="w-52 sm:w-40" label="Club" :items="clubs" :model-value="selectedClub"
            @update:model-value="newVal => (selectedClub = newVal)" />
          <SelectMenu class="w-52 sm:w-40" label="Contest type" :items="contestTypes" :model-value="selectedContestType"
            @update:model-value="newVal => (selectedContestType = newVal)" />
          <div class="w-52">
            <SelectMenu v-if="typeof selectedContest !== 'undefined'" class="w-full" label="Contest name"
              :items="selectedClubContests" :model-value="selectedContest"
      ...
      <div v-if="registrationData.length === 0"
